...
const UserManagement = () => {
  ...
  const [editUser, setEditUser] = useState(null); // user đang chỉnh sửa
  ...
  useEffect(() => {
    fetchUtil.get("/admin/dashboard-overview").then((data) => {
      setStatistics({
        totalUsers: data.totalUsers,
        activeUsers: data.activeUsers || data.totalUsers, // fallback nếu chưa có activeUsers
        newUsersToday: data.newUsersToday || data.newCustomersToday || 0,
      });
    });
  }, []);

  useEffect(() => {
    fetchUtil.get("/admin/role").then((res) => {
      setRoles(res.data || []);
    });
  }, []);
  ...
  const handleDelete = (id) => {
    fetchUtil.remove(`/admin/users/${id}`).then(fetchUsers);
  };
  ...
  const handleEditSubmit = (updatedUser) => {
    fetchUtil.put(`/admin/users/${updatedUser._id}`, updatedUser).then(() => {
                ...
                {filteredUsers
                  ...
                  .map((user) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {user.roleId?.name || "Chưa phân quyền"}
                      </TableCell>
      ...
      <Dialog open={openEditModal} onClose={() => setOpenEditModal(false)}>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth margin="normal">
            ...
            <Select
              labelId="role-label"
              value={editUser?.roleId?._id || editUser?.roleId || ""}
              label="Vai trò"
              onChange={(e) =>
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => handleEditSubmit(editUser)}
            variant="contained"
          >
            Lưu
          </Button>
